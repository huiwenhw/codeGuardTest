...
public class PipeCommand extends CallCommand {
	...
	/**
	 * Parses the command string and splits it into different commands based on
	 * pipe | operators
	 * 
	 * @return cmdVector Vector of String to store the split command string
	 *         into.
	 * 
	 ...
	 *             parsing.
	 */
	public Vector<String> parsePipe() throws ShellException {
		String[] patterns = { PATTN_DASH, PATTN_UQ, PATTN_DQ, PATTN_SQ, PATTN_BQ, PATTN_BQ_IN_DQ, PATTN_PIPE };
		String substring;
		String str = " " + cmdline + " ";
		int newStartIdx = 0, smallestStartIdx, smallestPattIdx, newEndIdx = 0;
		int cmdStartIdx = 0;
		boolean flag = false; // added this to connect 2 if stmts
	...
	private void cmdEndCheck(String checkStr) throws ShellException {
		if (cmdline.length() > 0 && cmdline.substring(cmdline.length() - 1).equals(checkStr)) {
			error = true;
